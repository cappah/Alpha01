...
namespace gearengine
{
	namespace graphics
	{
		...
		void TTexLoader::CreateEmptyTexture(int a_iWidth, int a_iHeight, GLenum format)
		{
		...
		void TTexLoader::CreateFromData(BYTE* bData, int a_iWidth, int a_iHeight, int a_iBPP, GLenum format, bool bGenerateMipMaps)
		{
		...
		bool TTexLoader::LoadTexture2D(std::string a_sPath, bool bGenerateMipMaps)
		{
		...
		void TTexLoader::SetSamplerParameter(GLenum parameter, GLenum value)
		{
		...
		void TTexLoader::SetFiltering(int a_tfMagnification, int a_tfMinification)
		{
		...
		void TTexLoader::BindTexture(int iTextureUnit)
		{
		...
		int TTexLoader::GetMinificationFilter()
		{
			return tfMinification;
		}

		int TTexLoader::GetMagnificationFilter()
		{
			return tfMagnification;
		}

		int TTexLoader::GetWidth()
		{
			return iWidth;
		}

		int TTexLoader::GetHeight()
		{
			return iHeight;
		}

		int TTexLoader::GetBPP()
		{
			return iBPP;
		}

		GLuint TTexLoader::GetTextureID()
		{
			return uiTexture;
		}

		std::string TTexLoader::GetPath()
		{
			return sPath;
		}

		bool TTexLoader::ReloadTexture()
		{
			FREE_IMAGE_FORMAT fif = FIF_UNKNOWN;
			FIBITMAP* dib(0);
		...
		TTexLoader tTextures[NUMTEXTURES];

		void LoadAllTextures()
		{
			// Load textures

			std::string sTextureNames[] = { "fungus.dds", "sand_grass_02.jpg", "rock_2_4w.jpg", "sand.jpg", "path.png" };

			FOR(i, NUMTEXTURES)
			{
				tTextures[i].LoadTexture2D("data\\textures\\ground\\natural\\" + sTextureNames[i], true);
